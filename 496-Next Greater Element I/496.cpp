...
public:
    vector<int> nextGreaterElement(vector<int>& findNums, vector<int>& nums) {
        vector<int> stk, answer(findNums.size());
        unordered_map<int, int> map;
        for (int i : findNums)map[i] = -1;
        for (int i = nums.size() - 1; i > -1; --i) { 
            ...
            if (!stk.empty()) {
                ...
                if (iter != map.end())
                    iter->second = stk.back();
            }
            stk.push_back(nums[i]);
        }
        for (int i = 0; i < findNums.size(); ++i)
            answer[i] = map[findNums[i]];
        return answer;
    }
};
int main() {
    Solution s;
    vector<int> vec = { 4,1,2 }, nums = { 1,3,4,2 };
    for (int i : s.nextGreaterElement(vec,nums))
        cout << i << "  ";
    return 0;
}
