    ...
    bool PredictTheWinner(vector<int>& nums) {
        ...
        for (int i = 1; i < len; ++i) {
            for (int j = 0; i + j < len; ++j) {
                pair<int, int> left = make_pair(nums[j] + dp[j + 1][j + i].second, dp[j + 1][j + i].first);
                pair<int, int> right = make_pair(nums[i + j] + dp[j][i + j - 1].second, dp[j][i + j - 1].first);
                dp[j][j + i] = left.first > right.first ? left : right;
            }
        }
        return !(dp[0].back().first < dp[0].back().second);
    }
};
int main() {
    Solution s;
    vector<int> vec{ 1, 5, 233, 7 };
    cout << boolalpha << s.PredictTheWinner(vec) << endl;
    return 0;
}
