...
skyline, and always has zero height. Also, the ground in between any two adjacent buildings should be considered 
...
class Solution {
public:
    //From:http://www.cnblogs.com/easonliu/p/4531020.html
    //Good solution
    vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
        vector<pair<int, int>> height, answer;
        for (auto &vec : buildings){
            height.push_back(make_pair(vec[0], -vec[2]));
            height.push_back(make_pair(vec[1], vec[2]));
        }
        ...
        for (auto &h : height){
            ...
            cur = *set.rbegin();
            if (cur != pre){
                answer.push_back(make_pair(h.first, cur));
                pre = cur;
            }
        }
        return answer;
...
int main(){
    Solution s;
    vector<vector<int>> vec = { { 2, 9, 10 }, { 3, 7, 15 }, { 5, 12, 12 }, { 15, 20, 10 }, { 19, 24, 8 } };
    for (auto &p : s.getSkyline(vec))
